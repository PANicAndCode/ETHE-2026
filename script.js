...
const STORAGE_PREFIX = "eatons-easter-hunt-progress-v1";
...
async function pushSharedSettings(){
  ...
  await supabaseClient.from("team_progress_eatons").upsert(payload, { onConflict: "team_id" });
}
...
async function fetchLeaderboard(){
  if (!supabaseReady) return;
  const { data, error } = await supabaseClient.from("leaderboard_eatons").select("*");
  if (error){
    console.error(error);
    supabaseReady = false;
    if (el("leaderboardModeText")){
...
async function fetchAllRemoteProgress(){
  if (!supabaseReady) return;
  const { data, error } = await supabaseClient.from("team_progress_eatons").select("*");
  if (error){
    console.error(error);
    return;
  }
...
async function loadRemoteProgress(team){
  ...
  const { data, error } = await supabaseClient.from("team_progress_eatons").select("*").eq("team_id", team).maybeSingle();
  if (error){
    console.error(error);
    return null;
  }
...
async function pushRemoteProgress(){
  ...
  const { error } = await supabaseClient.from("team_progress_eatons").upsert(payload, { onConflict: "team_id" });
...
async function pushLeaderboard(){
  ...
  const { error } = await supabaseClient.from("leaderboard_eatons").upsert(payload, { onConflict: "team_id" });
  if (error) console.error(error);
}
...
async function adminSaveTeamName(){
  ...
  if (supabaseReady){
    await supabaseClient.from("team_progress_eatons").upsert({
      team_id: team,
      team_name: newName,
    ...
    await supabaseClient.from("leaderboard_eatons").upsert({
      team_id: team,
      team_name: newName,
